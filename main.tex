...
To validate generalizability beyond the single in situ species, the visual branch was additionally re-trained and evaluated on the PlantVillage dataset ($n = 54{,}303$ leaf images spanning 14 species), and the soil-telemetry branch was calibrated using real NPK sensor measurements from 22 crop types (Kaggle Crop Recommendation), real continuous $CO_2$ flux time series from the COSORE database, and real volumetric soil-moisture time series from the USDA Cook Agronomy Farm sensor network.
...
To confirm that the late-fusion architecture provides diagnostic benefit beyond either individual modality, an ablation study was conducted comparing three model variants: (1) CNN-only, using only the visual branch; (2) RF-only, using only the soil telemetry branch; and (3) the full AgriDefend late-fusion model.
Because the in situ \textit{Raphanus} dataset was not separately re-partitioned for a per-branch ablation, this comparison was performed on the multi-species generalization dataset (PlantVillage, $n = 54{,}303$). On that dataset the CNN-only variant achieved an overall accuracy of 98.9\% and the RF-only variant 79.8\%, while the full late-fusion meta-learner reached 99.7\% ($R^2 = 0.99$, MAE\,=\,0.01), confirming that the combination of both modalities yields a diagnostic improvement over either branch individually (fusion $\geq$ best single modality). These accuracies derive from curated laboratory leaf imagery and are correspondingly higher than the in situ field result reported above; the relative ordering of the three variants, rather than their absolute magnitude, is the salient ablation outcome.
Independent validation of the RS-485 NPK readings against a King Conservation District soil-laboratory assay yielded a measurement error margin of 12--15\%, consistent with the $\pm$15\% commercial-off-the-shelf sensor tolerance assumed during feature engineering (Section~2.4).
